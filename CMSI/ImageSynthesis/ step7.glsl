...
\f0\fs24 \cf0 const int Steps = 1000;\
...
\{\
    p.y = 0.8*noise(frequence * vec2(p.x,p.z));\
    for(float j=1.0; j<octaves;j++) \{\
        p.y += pow(att,j) * 0.8 * \
            ridged(frequence * pow(2.0,j) * vec2(p.x,p.z));\
    \}\
    return p;\
\}\
\
...
\
   ...
   c = (1.0-fog)*c+fog*vec3(1.0,1.0,1.0);\
    \
   vec3 darkColour = vec3(0.0, 0.0, 0.0);\
   if(mod(p.y,0.2)<0.01) c = darkColour;\
    \
   return c;\
\}\
\
